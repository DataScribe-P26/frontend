...
const LabelManager = () => {
  ...
  const handleUpdateLabel = (updatedLabel) => {
    const isDuplicate = labels.some(
      (label) => label.name === updatedLabel.name && label.name !== currentLabel.name
    );
    ...
    const updatedLabels = labels.map((label) =>
      label.name === currentLabel.name ? { ...label, name: updatedLabel.name } : label
    );
  ...
  return (
    <div className={`flex flex-col h-screen overflow-hidden ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
      <Navbar />
      <div className="flex flex-grow">
        ...
        <div className={`flex-grow p-8 ${isDarkMode ? 'bg-gray-800' : 'bg-gradient-to-r from-gray-50 to-gray-100'}`}>
          ...
          <button
            ...
            className={`bg-purple-700 text-white mt-14 mb-14 px-4 py-2 rounded-lg font-semibold hover:bg-purple-600 transition-shadow shadow-lg hover:shadow-xl mb-4 ${isDarkMode ? 'bg-purple-600' : ''}`}
          >
            Create Label
          </button>
          {labels.length > 0 ? (
            <div className="overflow-auto items-center h-[30rem] scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
              <table className={`min-w-full border ${isDarkMode ? 'border-gray-600' : 'border-gray-300'} text-sm text-left ${isDarkMode ? 'bg-gray-700' : 'bg-white'} shadow-md rounded-lg`}>
                <thead>
                    ...
                    <tr key={index} className={`border-b ${isDarkMode ? 'border-gray-600' : 'border-gray-300'}`}>
                      ...
                      <td className="p-2 text-center flex justify-center space-x-2">
                        <button
                          ...
                          className={`text-yellow-500 p-2 hover:text-yellow-600 transition ${isDarkMode ? 'hover:text-yellow-400' : ''}`}
                        >
                          <FontAwesomeIcon icon={faEdit} />
                        </button>
                        <button
                          onClick={() => deleteLabel(label.name)}
                          className={`text-red-500 p-2 hover:text-red-600 transition ${isDarkMode ? 'hover:text-red-400' : ''}`}
                        >
