...
const handleFileChange = (event) => {
  ...
  if (!allowedFileTypes.includes(selectedFile.type) && !allowedExtensions.includes(`.${fileExtension}`)) {
    alert("Uploaded file format not supported. Please upload .txt, .json, .jsonl, or .csv files.");
    fileInput.value = "";
    return;
  }
        ...
        <div className="mb-4">
          ...
          >
            ...
            {projectType === "sentiment_analysis" && (
              <>
                <option value="jsonl" className={`${isDarkMode ? 'bg-gray-700 text-white' : 'bg-white text-gray-800'}`}>
                  JSON Line (.jsonl)
                </option>
                <option value="csv" className={`${isDarkMode ? 'bg-gray-700 text-white' : 'bg-white text-gray-800'}`}>
                  CSV File (.csv)
                </option>
              </>
            )}
  ...
  const renderAlreadyUploadedPage = () => (
    <div className={`flex-grow p-8 ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-gradient-to-r from-gray-100 to-gray-150'} flex flex-col justify-between items-center text-center`}>
      <div>
        ...
        <button
          onClick={projectType === "ner_tagging" ? handleGoToWorkspace : handleWorkspace}
