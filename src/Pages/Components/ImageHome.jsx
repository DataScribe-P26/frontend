...
function Imagehome() {
    ...
    const fetchImages = async () => {
      try {
        ...
        if (response.data.length > 0) {
          ...
          console.log('ABC Points:', formattedImages[0].segmentation_annotations),
  ...
  async function submit() {
    ...
    const segmentation_annotations = currentImage.annotations.filter(
      (a) => a.type === "segmentation"
    );
    console.log("rec", rectangle_annotations);
    console.log("poly", polygon_annotations);
    console.log("seg", segmentation_annotations);
